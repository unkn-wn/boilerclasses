...
export function useMediaQuery(q: string, init: boolean=true) {
...
export function AppTooltip({content, children, placement, className, onChange, ...props}: {content: React.ReactNode, placement: TooltipPlacement, onChange: (x: boolean)=>void}&HTMLAttributes<HTMLDivElement>) {
	...
	return <Tooltip placement={placement} key={placement} content={
			<IsInTooltipContext.Provider value={true} >{content}</IsInTooltipContext.Provider>
		}
		isOpen={reallyOpen==app.tooltipCount} onPointerEnter={interact} onPointerLeave={unInteract} >
